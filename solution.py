...
def anchor_sizes(boxes_list):
    b = np.concatenate(boxes_list)
    side = np.sqrt((b[:, 2] - b[:, 0]) * (b[:, 3] - b[:, 1]))
    base = int(2 ** np.floor(np.log2(np.percentile(side, ANCHOR_PCT))))
    return tuple(base * 2 ** k for k in range(5))


def build_model(anchors):
    ...
    model.rpn.anchor_generator = AnchorGenerator((anchors,) * 3, ((0.5, 1.0, 2.0),) * 3)
...
def crop_backbone():
    ...
    return nn.Sequential(m.conv1, m.bn1, m.relu, m.maxpool, m.layer1, m.layer2, m.layer3, m.layer4)


class BoxRefiner(nn.Module):
    def __init__(self):
        super().__init__()
        self.body = crop_backbone()
        k = REF_R // 32
        self.head = nn.Sequential(nn.Flatten(), nn.Linear(512 * k * k, 512), nn.ReLU(), nn.Linear(512, 4 * REF_BINS + 1))
        self.register_buffer("bins", torch.linspace(-1, 1, REF_BINS))

    def raw(self, x):
        o = self.head(self.body(x))
        return o[:, :4 * REF_BINS].reshape(-1, 4, REF_BINS), o[:, 4 * REF_BINS]

    def forward(self, x):
        logits, q = self.raw(x)
        return torch.cat([(logits.softmax(-1) * self.bins).sum(-1), q[:, None]], 1)
...
class CropScorer(nn.Module):
    def __init__(self):
        ...
        self.body = crop_backbone()
        k = REF_R // 32
        self.head = nn.Sequential(nn.Flatten(), nn.Linear(512 * k * k, 512), nn.ReLU(), nn.Linear(512, 1))

    def forward(self, x):
        return self.head(self.body(x))[:, 0]


def bin_loss(logits, y, m):
    pos = (y.clamp(-1, 1) + 1) / 2 * (REF_BINS - 1)
    lo = pos.floor().clamp(0, REF_BINS - 2)
    wl = 1 - (pos - lo)
    lo = lo.long()
    logp = logits.log_softmax(-1)
    nll = -(logp.gather(-1, lo[..., None]).squeeze(-1) * wl + logp.gather(-1, (lo + 1)[..., None]).squeeze(-1) * (1 - wl)).sum(1)
...
def train_refiner(imgs, boxes_list, rng):
    ...
    for st in range(REF_STEPS):
        ...
        logits, qlog = net.raw(X)
        m = (Q > 0.3).float()
        lreg = bin_loss(logits, Y, m)
        lq = F.binary_cross_entropy_with_logits(qlog, Q)
        loss = lreg + lq
        opt.zero_grad()
        loss.backward()
        opt.step()
...
def train_scorer(imgs, pool, boxes_list, rng):
    net = CropScorer().to(DEVICE)
    net.train()
    opt = torch.optim.AdamW([p for p in net.parameters() if p.requires_grad], lr=REF_LR, weight_decay=1e-4)
    run = 0.0
    for st in range(SCO_STEPS):
        lr = REF_LR * min(1.0, (st + 1) / WARM) * 0.5 * (1 + math.cos(math.pi * st / SCO_STEPS))
        for pg in opt.param_groups:
            pg["lr"] = lr
        X, Y = [], []
        for pj in rng.integers(len(pool), size=REF_IMGS):
            i, bx = pool[pj]
            b = bx[rng.integers(len(bx), size=REF_PER_IMG)].astype(np.float64)
            wh = np.maximum(b[:, 2:] - b[:, :2], 4)
            b = b + rng.normal(0, SCO_JITTER, b.shape) * np.concatenate([wh, wh], 1)
            y = iou_matrix(b, boxes_list[i]).max(1) >= SCO_POS_IOU
            k = int(rng.integers(8))
            X.append(dihedral_crops(crop_regions(photometric(to_tensor(imgs[i]), rng), b), k))
            Y.append(y)
        X = torch.cat(X).to(DEVICE)
        Y = torch.from_numpy(np.concatenate(Y)).float().to(DEVICE)
        ...
        run += float(loss)
        if (st + 1) % 50 == 0:
            print(f"scorer step {st + 1}/{SCO_STEPS} bce {run / 50:.4f}", flush=True)
            run = 0.0
    net.eval()
    return net
...
@torch.no_grad()
def score_boxes(net, img, b):
    if len(b) == 0:
        return np.zeros(0)
    x = crop_regions(to_tensor(img), b)
    n = len(b)
    out = torch.sigmoid(net(torch.cat([dihedral_crops(x, k) for k in REF_VARIANTS]).to(DEVICE))).cpu().numpy().astype(np.float64)
    return out.reshape(len(REF_VARIANTS), n).mean(0)


def candidates(refiner, scorer, img, det):
    bx, sc = det
    m = sc >= REF_MIN_SCORE
    bx, sc = bx[m], sc[m]
    rb, q = refine_boxes(refiner, img, bx)
    return bx, sc, rb, score_boxes(scorer, img, rb)


def variant(cand, use_ref, power):
    bx, sc, rb, p = cand
    return (rb if use_ref else bx), sc * np.power(np.clip(p, 1e-6, 1.0), power)
...
def main():
    ...
    anchors = anchor_sizes([all_boxes[i] for i in tr_idx])
    print(f"anchor sizes from train boxes {anchors}", flush=True)
    ...
    pool_rows = np.arange(0, len(tr_idx), POOL_EVERY)
    tr_dets = predict(model, tr_imgs[tr_idx[pool_rows]])
    ...
    pool = []
    for j, (bx, sc) in zip(pool_rows, tr_dets):
        m = sc >= REF_MIN_SCORE
        if m.any():
            pool.append((int(j), bx[m]))
    print(f"scorer pool images {len(pool)} candidates {sum(len(b) for _, b in pool)}", flush=True)
    scorer = train_scorer(tr_imgs[tr_idx], pool, [all_boxes[i] for i in tr_idx], rng)
    print("scorer training done", flush=True)
    ho_cand = [candidates(refiner, scorer, tr_imgs[i], d) for i, d in zip(ho_idx, ho_dets)]
    te_cand = [candidates(refiner, scorer, te_imgs[i], d) for i, d in enumerate(te_dets)]
    ...
    for use_ref in (0, 1):
        for power in SCORE_POWERS:
            vs = [variant(c, use_ref, power) for c in ho_cand]
            local = None
                ...
                for force in (0, 1):
                    for thr in thrs:
                        ...
                        if local is None or f1 > local[0] + 1e-12:
                            local = (f1, use_ref, power, sup, force, float(thr), tp, p)
            print(f"refined {use_ref} scorer power {power}: F1@{MATCH_IOU} {local[0]:.4f} cfg {local[3:]}", flush=True)
            if best is None or local[0] > best[0] + 1e-12:
                best = local
    f1, use_ref, power, sup, force, thr, tp, p = best
    print(f"holdout detection F1@{MATCH_IOU} {f1:.4f} refined {use_ref} scorer power {power} suppress {sup} force {force} thr {thr} tp {tp} pred {p} targets {total_t}", flush=True)

    rows = []
    for i in range(len(test)):
        fb, fs = variant(te_cand[i], use_ref, power)
